/**
 * 10x Team Universal Browser Controller - Content Script
 *
...
console.log(`[10x Team Content] Loaded on ${currentPlatform} platform`);
...
function trackActivity(type, data = {}) {
  ...
  console.log(`[10x Team Content] Activity tracked: ${type}`, data);
...
function flushActivityBuffer() {
  ...
  chrome.runtime.sendMessage({
    ...
    console.error('[10x Team Content] Failed to send activities:', error);
  });
}
...
function extractData(selectors) {
  ...
  for (const [key, selector] of Object.entries(selectors)) {
    try {
      ...
      console.error(`[10x Team Content] Error extracting ${key}:`, error);
      data[key] = null;
    }
  }
...
async function clickElement(selector, options = {}) {
  ...
  try {
    ...
    console.error(`[10x Team Content] Click failed:`, error);
    throw error;
  }
}
...
async function typeText(selector, text, options = {}) {
  ...
  try {
    ...
    console.error(`[10x Team Content] Type failed:`, error);
    throw error;
  }
}
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  console.log(`[10x Team Content] Received command:`, message.type);
  ...
  (async () => {
    try {
      ...
      console.error(`[10x Team Content] Command failed:`, error);
...
console.log('[10x Team Content] Content script initialized');

// Check connection to background worker
chrome.runtime.sendMessage({ type: 'GET_CONNECTION_STATUS' })
  .then(response => {
    console.log('[10x Team Content] Background worker connection:', response.connected ? '✓' : '✗');
  })
  .catch(error => {
    console.error('[10x Team Content] Failed to connect to background worker:', error);
  });
