/**
 * 10x Team Universal Browser Controller - Background Service Worker
 *
...
console.log('[10x Team Browser] Extension loaded');
connectToWebSocket();
...
function connectToWebSocket() {
  if (ws && (ws.readyState === WebSocket.CONNECTING || ws.readyState === WebSocket.OPEN)) {
    console.log('[10x Team Browser] Already connected or connecting');
    return;
  }

  console.log(`[10x Team Browser] Connecting to ${CONFIG.WEBSOCKET_URL}...`);
  ...
  try {
    ...
    console.error('[10x Team Browser] WebSocket connection error:', error);
    scheduleReconnect();
  }
}
...
function handleWebSocketOpen() {
  console.log('[10x Team Browser] ✅ Connected to Canvas WebSocket');
  isConnected = true;
  reconnectAttempts = 0;
...
async function handleWebSocketMessage(event) {
  try {
    ...
    console.log('[10x Team Browser] Received command:', message.type);
    ...
    switch (message.type) {
      ...
      default:
        console.warn('[10x Team Browser] Unknown message type:', message.type);
    }

  } catch (error) {
    console.error('[10x Team Browser] Error handling message:', error);
    sendToWebSocket({
      type: 'error',
      error: error.message,
...
function handleWebSocketError(error) {
  console.error('[10x Team Browser] WebSocket error:', error);
  isConnected = false;
...
function handleWebSocketClose(event) {
  console.log('[10x Team Browser] WebSocket closed:', event.code, event.reason);
  isConnected = false;
...
function scheduleReconnect() {
  if (reconnectAttempts >= CONFIG.MAX_RECONNECT_ATTEMPTS) {
    console.error('[10x Team Browser] Max reconnect attempts reached');
    return;
  }
  ...
  console.log(`[10x Team Browser] Reconnecting in ${delay}ms (attempt ${reconnectAttempts}/${CONFIG.MAX_RECONNECT_ATTEMPTS})`);
...
function sendToWebSocket(message) {
  if (!ws || ws.readyState !== WebSocket.OPEN) {
    console.warn('[10x Team Browser] WebSocket not connected, queueing command');
  ...
  try {
    ...
    console.error('[10x Team Browser] Error sending message:', error);
    pendingCommands.push(message);
    return false;
  }
}
...
function processPendingCommands() {
  ...
  console.log(`[10x Team Browser] Processing ${pendingCommands.length} pending commands`);
...
async function executeBrowserCommand(command) {
  console.log('[10x Team Browser] Executing command:', command.action);
  ...
  try {
    ...
    console.error('[10x Team Browser] Command failed:', error);
...
async function executeLinkedInAction(action) {
  console.log('[10x Team Browser] LinkedIn action:', action.type);
...
async function executeInstagramAction(action) {
  console.log('[10x Team Browser] Instagram action:', action.type);
...
async function executeTwitterAction(action) {
  console.log('[10x Team Browser] Twitter action:', action.type);
...
async function executeGoogleAction(action) {
  console.log('[10x Team Browser] Google action:', action.type);
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  console.log('[10x Team Browser] Message from content script:', message.type);
  ...
  switch (message.type) {
    ...
    case 'RECONNECT':
      ...
      console.log('[10x Team Browser] Manual reconnect requested');
      if (ws) {
        ws.close();
      }
      reconnectAttempts = 0;
...
chrome.runtime.onInstalled.addListener((details) => {
  console.log('[10x Team Browser] Extension installed:', details.reason);
