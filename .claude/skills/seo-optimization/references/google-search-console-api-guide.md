...
## Quick Start (10x Team Scripts)

10x Team provides cross-platform scripts for GSC integration. Use these for most tasks:
...
**Config location:** `.claude/secrets/google_client_secret.json` (project) or `~/.claude/secrets/` (global)
...
8. Place in .claude/secrets/ (project scope) or ~/.claude/secrets/ (global)
```
...
**Step 3: Authenticate with 10x Team Script**
```bash
